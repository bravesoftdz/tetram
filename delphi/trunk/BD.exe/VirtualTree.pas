...
*)
    ...
    INITIALEFIELDS: 'INITIALENOMUNIVERS'; INITIALEVALUE: 'INITIALENOMUNIVERS'; REFFIELDS: 'ID_UNIVERS'; TABLESEARCH: 'vw_liste_univers';
  ...
  begin
    ...
    with q do
      try
        ...
        Open;
        vmModeInfos[FMode].ClassPointeur.FillList(InfoNode.List, q);
      finally
        Transaction.Free;
        Free;
      end;
...
begin
  ...
  Node := GetFirstSelected;
  if GetNodeLevel(Node) > 0 then
    if GetNodeBasePointer(Node) is TAlbum then
      Result := TAlbum(GetNodeBasePointer(Node)).ChaineAffichage(True)
