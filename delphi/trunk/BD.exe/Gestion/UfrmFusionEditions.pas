...
begin
  ...
  begin
    ...
    if Edition.Gratuit then
      ...
      Label4.Caption := ICUFormatCurrency(Edition.Prix);

    if Edition.PrixCote > 0 then
      Label38.Caption := Format('%s (%d)', [ICUFormatCurrency(Edition.PrixCote), Edition.AnneeCote])
    else
      Label38.Caption := '';
  end;
end;
...
begin
  ...
  if Edition.Gratuit then
    ...
    Prix.Caption := ICUFormatCurrency(Edition.Prix);

  if Edition.PrixCote > 0 then
    lbCote.Caption := Format('%s (%d)', [ICUFormatCurrency(Edition.PrixCote), Edition.AnneeCote])
  else
    lbCote.Caption := '';
end;
