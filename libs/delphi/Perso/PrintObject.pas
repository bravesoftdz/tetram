...
type
  ...
  IPrintObjectPreview = interface
    ...
    procedure Start; safecall; // d�but de l'impression
  ...
  private
    ...
    OPreview: IPrintObjectPreview; { Objet r�cup�rant toutes les actions pour pr�view }
    FDateTime: TDateTimeRecord; { Informations d'impression de la date }
    FAutoPaging: Boolean; { Passage � la page automatique? }
    FFont: TFont; { Font par d�faut }
    FMargin: TMarginsPixels; { Margins in pixels }
    FMarginMm: TMarginsMms; { Margins in milimeters }
    FHeader: THeaderRecords; { Informations d'impression du T�te de page }
    ...
    FDetail: TDetailCoordinates; { Coordonn�es du Detail }
    FHeaderCoordinates: THeaderCoordinates; { Coordonn�es du T�te de page }
    FFooterCoordinates: TFooterCoordinates; { Coordonn�es du Pied de page }
    FPageNumber: TPageNumberRecord; { Informations d'impression du num�ro de page }
    FSizeOption: TSizeOption; { Option de calcul de la taille de la feuille d'apper�u }
    ...
    StartDateTimePrint: TDateTime; { D�but d'impression }
    CurrentTab: Single; { The value of the current tab }
    CurrentFont: TCurrentFontRecords; { Historique des Font en cours d'utilisation }
    PrevFont: TFont; { Derni�re Font utilis�e pour imprimer }
    FTitre: string; { Titre de la fen�tre d'apper�u ou du gestionnaire d'impression }
...
procedure TPrintObject.SetMargins(Top, Bottom, Left, Right: Single); { D�fini les marges haute, basse, gauche et droite en MMS ! }
begin
...
begin
  ...
  //    else begin
  //        LastSize := Canvas.Font.Height; // plus pr�cis que Size
...
procedure TPrintObject.WriteLine(X, Y: Single; Text: string); { Ecrit Text � la position X, Y (en MM). }
var
  XPixels: Integer;
  YPixels: Integer;
begin
...
procedure TPrintObject.WriteRotatedLine(X, Y: Single; Text: string; Font: TFont; Angle: Integer); { Ecrit Text � la position X, Y (en MM). }
...
procedure TPrintObject.Quit; { 'Quit' doit �tre appel�e pour valider l'impression }
var
  Cancel: Boolean;
begin
  CheckPrinting;
...
//  LastSize: Integer;
begin
  Destination.Font.Assign(Font); // bugg�: ne transmet pas l'Escapement !!!!
  // correction: pas bugg�: TFont n'utilise pas l'Escapement
...
procedure TPrintObject.Start(Title: string); { doit TOUJOURS �tre appel�e avant de commencer une impression }
var
  Cancel: Boolean;
begin
