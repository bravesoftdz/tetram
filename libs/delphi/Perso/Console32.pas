...
{ affiche mess en positions x,y de l'�cran}
...
{ met l'�cran dans les couleurs texte et fond d'�cran}
procedure Couleur_Ecran(const text, fond: COULEUR);

{ sauve les couleurs texte et fond d'�cran}
procedure Sauve_Couleur;

{ restaure les couleurs texte et fond d'�cran}
procedure Restaure_Couleur;

{ efface l'�cran}
procedure Efface_Ecran;

{pr�sente un  �cran  vierge en couleur de fond et text}
procedure Nouvel_Ecran(const text, fond: COULEUR);

{ efface la fin de ligne � partir du curseur}
procedure Efface_Fin_Ligne;

{ efface la fin de page � partir du curseur}
procedure Efface_Fin_Page;

{ sauve le contenu de l'�cran texte et couleurs � 10 niveaux}
procedure Sauve_Ecran;

{ restaure le dernier �cran}
procedure Restaure_Ecran;

{ saisie d'un caract�re sans l'afficher � l'�cran
 la fonction retourne RETKEY (AsciiChar+Scancode , utile pour les touches fonction).
 Nono40 a fortement contribu� � l'�laboration de cette fonction}
function LireKey: RETKEY;

{ Fonction donnant TRUE si une touche est en attente
 elle ne prend en compte que les touches donnant un caract�re, donc
 elle ne fonctionne pas avec les touches de fonctions.
 Nono40 a �labor� cette fonction}
function KeyAppuyee: Boolean;

{ ins�re une ligne � l'endroit du curseur}
procedure Insere_Ligne;

{ supprime une ligne � l'endroit du curseur}
procedure Supprime_Ligne;

{ temporise tant qu'une touche n'est pas appuy�e}
procedure Appuie;

{ met un libell� dans la barre de titre}
procedure Titre_Fenetre(const Titre: string);

{ transforme une chaine de caract�res ANSI (windows) en OEM (DOS)}
function Oem(const st: string): string;

{ retourne l'OS utilis� - 0 : Win3.11, 1 : Win9X, 2 : Win2k ou XP}
function VersionWindows: integer;

{ les 3 fonctions infra g�rent la capture d'�cran}
...
begin
  ...
  begin
    repeat
      { Lecture du buffer des �v�nement}
    ...
    { permet d'harmoniser la touche ESC diff�rents suivant OS }
...
var
  ...
  i: Cardinal;
begin
  {Demande de la liste des �v�nements en attente}
  PeekConsoleInput(Clavier_handle, Zone[1], TailleBuf, EnAttente);
  Result := False;
  i := 1;
  while not Result and (i <= EnAttente) and (i <= TailleBuf) do
  begin
    Result := (Zone[i].EventType = 1) { C'est un �v�nement clavier}
    and (Zone[i].Event.KeyEvent.bKeyDown) { C'est une touche appuy�e }
    and (Zone[i].Event.KeyEvent.AsciiChar <> #0); { ce n'est pas une touche de controle}
    Inc(i);
  end;
  { Si il n'y a pas de touche, le buffer est vid� car windows
   � une limite tr�s basse d'�v�nements en attente}
