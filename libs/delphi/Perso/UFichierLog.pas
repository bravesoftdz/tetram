  ...
  for fileName in TDirectory.GetFiles(TFichierLog.Instance.BackupPath, rootLogFileName + '*.gz', TSearchOption.soTopDirectoryOnly,
    ...
    begin
      try
        ...
        // si le nom de fichier n'est pas d�codable, c'est qu'il n'est pas un fichier de backup
        Result := False;
      end;
    end) do
...
begin
  ...
  begin
    ...
    try
      ...
      // on copie le BOM en m�me temps s'il est pr�sent
      FFichierLog.Position := 0;
      bckStream.CopyFrom(FFichierLog, cutPos);

      // FFichierLog a �t� repositionn� sur cutPos
...
begin
  if TLogWriter(FWriter).isOpen then
    raise Exception.Create('Impossible de modifier les propri�t�s');
end;
...
begin
  if not(Value in [0 .. 99]) then
    raise Exception.Create('Le ratio doit �tre compris entre 0% et 99%');
  FKeepRatio := Value;
end;
